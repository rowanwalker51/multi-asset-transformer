...
def load_raw_data() -> Dict[str, pd.DataFrame]:
    ...
    rf_file = "rf.parquet"
    rf = (
        pd.read_parquet(raw_paths["rf"] / rf_file)
    ...
    gbp_usd = (
        pd.read_parquet(raw_paths["fx"] / gbp_usd_file)
    ...
    gbp_eur = (
        pd.read_parquet(raw_paths["fx"] / gbp_eur_file)
    ...
    gold = (
        pd.read_parquet(raw_paths["commodities"] / gold_file)
    ...
    oil = (
        pd.read_parquet(raw_paths["commodities"] / oil_file)
...
def generate_valid_tickers(
    ...
    all_tickers = (
        pd.read_parquet(all_tickers_path / all_tickers_file)["ticker"]
        .to_list()
    )
...
def generate_model_inputs(
    ...
    for ticker_id, ticker in enumerate(tickers):
        ...
        for i in range(len(train_df) - seq_len - hold_days):
            ...
            seq = (seq - seq.mean(axis=0)) / np.clip(
                seq.std(axis=0),
                1e-5,
                None,
            )
            ...
            regime_X.append(
                train_df[regime_cols].iloc[i:i + seq_len].values
            )
