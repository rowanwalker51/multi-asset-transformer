...
app.layout = html.Div(
    [
        html.H1(
            "Multi-Asset Transformer Backtest",
...
def update_backtest(
    ...
    strategy_eq, benchmark_eq, rf_series = backtest(
    ...
    rolling_sharpe_series = rolling_sharpe(strategy_returns)

    risk_df = full_risk_report(
        strategy_eq,
        benchmark_eq,
        rf_series,
    ).T
    ...
    fig.add_trace(
        go.Scatter(
            x=benchmark_eq.index,
            y=benchmark_eq.values,
            mode="lines",
            name="Benchmark",
        ),
        row=1,
    ...
    fig.update_layout(
        title={
            "text": "Strategy Backtest Performance",
            "x": 0.5,
            "xanchor": "center",
            "font": {"size": 24},
        },
